...
class SockBackend:
    def __init__(self, server_no, config):
        self.inbox = queue.Queue()
        self.host, self.port = config[server_no]
        self._server_no = server_no
        self._logger = logging.getLogger(f'RaftServer-{server_no}')
        self._config = config
        self._connections = [None for _ in range(len(config))]
    ...
    def _accept_clients(self):
        ...
        sock.listen(1)
        self._logger.info(f'Server {self._server_no} running at {self.host}:{self.port}')
        while True:
    ...
    def _handle_client(self, client):
        while True:
            ...
            self._logger.info(f'received message of {len(raw_msg)} bytes from server {msg.server_no}')
            self.inbox.put(msg)
    ...
    def send(self, server_no, msg):
        ...
        try:
            ...
            self._logger.info(f'sending a message to {server_no} using a cached connection')
        except (AttributeError, ConnectionRefusedError):
            self._logger.info(f'no cached connection for server {server_no}, connecting to {self._config[server_no]}')
            s = socket(AF_INET, SOCK_STREAM)
            s.connect(self._config[server_no])
